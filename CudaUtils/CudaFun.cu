...
void processImageWithCuda(const cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst) {
	...
	if(&src == &dst || src.size() != dst.size() || src.type() != dst.type()) {
		...
		processKernel<<<gridSize, blockSize>>>(src.ptr<uchar3>(), result.ptr<uchar3>(), width, height);
		cudaDeviceSynchronize();
		dst = std::move(result);
	} else {
		processKernel<<<gridSize, blockSize>>>(src.ptr<uchar3>(), dst.ptr<uchar3>(), width, height);
		cudaDeviceSynchronize();
	}
}
